  ...
  <div className='bg-gray-700 p-6 rounded-xl shadow-md mb-6'>
    <div className='mb-2'>
      <span className='text-[1.75rem] text-[#10B981] font-semibold'>
        Total: {HelperService.formatCurrency(total ?? 0)}
      </span>
    ...
    <ResponsiveContainer width='100%' height={300}>
      <BarChart data={data}>
        ...
        <Tooltip
          ...
          formatter={(val: unknown) => HelperService.formatCurrency(Number(val))}
          contentStyle={{
        ...
        <Legend />
        {keys.map((key, i) => (
          <Bar key={key} dataKey={key} fill={colors[i]} name={`${key[0].toUpperCase()}${key.slice(1)} Orders`} />
        ))}
      </BarChart>
    </ResponsiveContainer>
  </div>
