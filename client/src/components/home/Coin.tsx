    ...
    return (
        <>
                ...
                <div className="text-left place-content-center">{asset.name}</div>
                    <div className="place-content-center">{HelperService.formatCurrency(asset.priceUsd)}</div>            
                    <div className={`${asset.changePercent24Hr > 0 ? "text-emerald-500" : "text-rose-500"} place-content-center`}>
                        {(Math.round(asset.changePercent24Hr * 1000) / 1000).toFixed(2)}%
                    </div>
                <TestChart asset_id={asset.id} limit={90}/>
                <div className="place-content-center"><Link to="/registration">Trade now -{'>'}</Link></div>
            </div>
        </>
    )
