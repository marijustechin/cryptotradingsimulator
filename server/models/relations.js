function modelRelations(sequelize) {
  const { user, user_secret, token, wallet, portfolio, cryptocurrencies, purchases, sales } = sequelize.models;
  ...
  user.belongsToMany(cryptocurrencies, {through: portfolio, foreignKey: 'user_id', onDelete: 'CASCADE'});
  cryptocurrencies.belongsToMany(user, {through: portfolio, foreignKey: 'crypto_id', onDelete: 'CASCADE'});

  purchases.belongsTo(user, {foreignKey: 'user_id', onDelete: 'CASCADE'});
  purchases.belongsTo(cryptocurrencies, {foreignKey: 'crypto_id', onDelete: 'CASCADE'});
  sales.belongsTo(user, {foreignKey: 'user_id', onDelete: 'CASCADE'});
  sales.belongsTo(cryptocurrencies, {foreignKey: 'crypto_id', onDelete: 'CASCADE'});
